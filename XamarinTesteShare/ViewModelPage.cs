...
namespace XamarinTesteShare
{
    public class ViewModelPage : BaseViewModel
    {
        bool shareText = true;
        bool shareUri;
        string text;
        ...
        async void OnFileRequest()
        {
            if (!string.IsNullOrWhiteSpace(ShareFileAttachmentContents))
            {
                ...
                var fn = string.IsNullOrWhiteSpace(ShareFileAttachmentName) ? "FileSystemTemplate.txt" : ShareFileAttachmentName.Trim();
